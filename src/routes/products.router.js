...
// 상품 상세 조회, 수정, 삭제 시 상품이 없는 경우에는 “상품이 존재하지 않습니다.” 메세지를 반환합니다. 
// 상품 목록 조회 시 상품이 없는 경우에는 빈 배열([])을 반환합니다.
// 상품 생성 시 입력 받은 상품명이 기존에 등록 된 상품명과 동일한 경우에는 “이미 등록 된 상품입니다.” 메세지를 반환합니다.
...
productsRouter.post('/products', async (req, res, next) => {
  try {
    ...
    // 1-2. 생성된 상품 정보를 클라이언트에게 응답(Response) 반환한다.
    // 201: 상품 생성에 성공했습니다.
    return res
      .status(201)
...
productsRouter.delete('/products/:id', async (req, res) => {
  try {
    ...
    // 5-3. 삭제할 상품과 비밀번호 일치 여부를 확인한 후, 동일할 때만 상품이 삭제되어야 합니다. 
    const productInfo = await products.findById(id, {password: true}).exec();
    const isPasswordMatched = password === productInfo.password;
